...
const reply_markup = {
    inline_keyboard: [
        [
            {
                text: '🔙 Назад',
                callback_data: 'b:lztadmin:settings'
            }
        ]
    ]
}
...
export default {
    ...
    async exec(message, args) {
        ...
        if(!args[0]) {
            ...
            states.set(message.from.id, { action: "lztadmin", args: [...args, message.raw]})
        ...
        await user.findOneAndUpdate({ id: message.from.id }, { $set: { admToken: token }})
